...
func main() {
	...
	flag.StringVar(&generate, "generate", "types,client,server,spec",
		`Comma-separated list of code to generate; valid options: "types", "client", "apigw", "chi-server", "server", "spec", "skip-fmt", "skip-prune"`)
	...
	if (opts.GenerateEchoServer && opts.GenerateChiServer) ||
		(opts.GenerateAPIGWServer && opts.GenerateChiServer) ||
		(opts.GenerateEchoServer && opts.GenerateAPIGWServer) {
		errExit("can not specify multiple types - server, chi-server or apigw targets simultaneously")
	}
